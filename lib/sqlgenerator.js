/**
   Class used to generate PostgreSQL compatible SELECT SQL queries.
    ...
    - Identifiers and values cannot contain single or double quotes;
    - Identifiers and values cannot contain ascii control characters (\x00-\x1F)
      (The null byte is not valid even in quoted identifiers; but there is no
       reason to accept any controlled characters).

    Ideally the escaping should be done by the PostgreSQL server; however this
    is not an option as we must pass an SQL query to Windshaft/mapnik.
...
Select.prototype.to_sql = function(){
  ...
  if (this._query.inner_join.length > 0){
    query.push('INNER JOIN ' + this._sql_section('inner_join').join(' INNER JOIN '));
  }
...
/**
 * Clean a string for use as a (quoted) field name or a value by ensuring:
 * - it only contains valid UTF-8;
 * - it does not contain single or double quotes;
 * - it does not contain control characters (in particular a null byte)
 *
 * Note that we *clean* the string rather than attempt to escape it. This
 * is slightly safer, though we might lose information.
 *
 * @param val: String to clean up
 * @return: String - the cleaned up string.
 */
Select.prototype._clean_string = function(val){
  val = val.replace(/[\0-\x1F'"]/g, '');
