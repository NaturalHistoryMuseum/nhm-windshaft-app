...
TileQuery.prototype.tile_query = function(callback){
  ...
  if (style == 'heatmap') {
    ...
    // no need to shuffle (see below), so no need for an outer query
    ...
    outer_q = new Select({'compact': true}, {
      geom_field_label: this._config.geometry_field
    });
  ...
  /* We need the MSS to find additional fields - so start by generating the MSS
     and finalize the SQL after that.
   */
  if (fs.existsSync(this._config.resources_path + '/' + this._resource_id + '/mss/' + style + '.mss')){
    mu.root = this._config.resources_path + '/' + this._resource_id + '/mss';
  } else {
  ...
  mu.compileAndRender(style + '.mss', this._style[style]).on('data', function(data) {
    ...
    var additional_fields = tq._find_mss_fields(mss);
    for (var i = 0; i < additional_fields['fields'].length; i++){
      var f = additional_fields['fields'][i];
      if (f != 'zoom' && f != 'count'){
        var type = '';
        if (typeof(additional_fields['types'][f]) !== 'undefined'){
            type = '::' + additional_fields['types'][f];
        }
        query.select('{field}' + type, {field: f});
        if (outer_q){
          outer_q.select('{field}', {field: f});
        }
      }
    }
    var sql = '';
...
/**
 * Return the list of fields that are referenced in an MSS string.
 *
 * @param mss: Style to parse the MSS of
 */
TileQuery.prototype._find_mss_fields = function(mss){
  // We assume identifiers conform to CSS standard - See http://www.w3.org/TR/CSS2/syndata.html#value-def-identifier
  // Hence the complicated matching and un-escaping.
  var re = /\[\s*((\\[0-9a-fA-F]{1,6} ?|\\.|[-_a-zA-Z0-9\u00A1-\uFFFF])+)\s*(=|!=|~=|<=|>=)(.+?)\]\s*[\[{]/gm;
  var additional_fields = [];
  var types = {};
  var match = re.exec(mss);
  while (match != null){
    var f = match[1];
    f = f.replace(/\\[0-9a-fA-F]{1,6} ?/g, function(f){
      return String.fromCharCode(f.replace('\\', '0x'));
    });
    f = f.replace(/\\./g, function(f){
      return f.replace(/^\\/, '');
    });
    // Attempt to detect field that should be cast to text fields.
    if (match[4].match(/\s*["']/)){
       types[f] = 'text';
    }
    if (additional_fields.indexOf(f) < 0) {
      additional_fields.push(f);
    }
    match = re.exec(mss);
  }
