...
TileQuery.prototype._base_query = function(marker_size, grid_size){
  ...
  if (this._params.filters){
    ...
    for (var f in filter_items){
      ...
      var value = n_v[1];
      if (name == '_tmgeom'){
        query.where("ST_Intersects({geom_field}, ST_Transform(ST_GeomFromText({geom}, 4326), 3857))", null, {
          geom: value
        });
      } else {
...
TileQuery.prototype.tile_query = function(callback){
  ...
  if (!(style in ['plot', 'gridded', 'heatmap'])){
    style = 'plot';
  }
...
TileQuery.prototype.grid_query = function(callback){
  ...
  if (this._params.interactivity) {
    for (var col in this._params.interactivity.split(',')) {
      outer_query.select('{col}', {
        'col': [this._resource_id, col]
      });
    }
  }
  ...
  if (this._params.interactivity){
    interactivity.concat(this._params.interactivity.split(','));
